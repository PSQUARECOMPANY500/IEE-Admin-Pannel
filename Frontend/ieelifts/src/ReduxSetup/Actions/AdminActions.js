...
export const assignCallBackByAdminAction = (
  ...
  return async (dispatch) => {
    try {
      ...
      const responcedata = await axios.put(
        `${config.apiUrl}/client/updateCallbacks`,
        {
          callbackId,
          name,
          enggJon,
...
export const requestLimitedClientData = (JobOrderNumber) => {
  return async (dispatch) => {
    try {
      const response = await axios.get(`${config.apiUrl}/admin/getClientMemberShipDataLimited`, {
        JobOrderNumber,
      });
      dispatch({
